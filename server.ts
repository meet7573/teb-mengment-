...
function containsPin(value: unknown, target: string, depth = 0): boolean {
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    ...
    // Legacy records used a 6-digit PIN-xxxxxx while the UI has always displayed the first 4 digits.
...
// Student registration uses the automatic 4-digit App PIN generated by the admin UI.
app.post('/api/student/register', async (req, res) => {
  ...
  try {
    ...
    const student = { id: studentId, name, pinHash: pinHash(pin), pinNumber: `PIN-${pin}`, standard, coachingType, roomNumber, wingNumber, assignedTabletId: null, isActive: true, status: 'Active', createdAt: now, updatedAt: now };
    ...
    return res.status(201).json({ student: { id: studentId, name, standard, coachingType, roomNumber, wingNumber, tabletId: null, pinNumber: `PIN-${pin}` }, appPin: pin });
...
// Student logs in only with the automatic 4-digit App PIN. The tablet comes exclusively from Admin assignment.
app.post('/api/student/activate', async (req, res) => {
  ...
  try {
    ...
    if (!student || student?.isActive === false || String(student?.status ?? 'Active').toLowerCase() === 'inactive') return res.status(401).json({ error: 'Student PIN is incorrect or inactive.' });
