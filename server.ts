...
app.post('/api/student/activate', async (req, res) => {
  ...
  try {
    const [students, sessions] = await Promise.all([readCollectionServerSide('students'), readCollectionServerSide('studentSessions')]); const student = students.find((item) => containsPin(item, pin));
    ...
    const sessionToken = randomUUID(); const startedAt = new Date().toISOString(); const session = { id: sessionToken, sessionTokenHash: hashToken(sessionToken), studentId, tabletId: assignedTabletId, studentName: String(student?.name ?? student?.studentName ?? 'Student').trim() || 'Student', startedAt, returnedAt: null, durationMinutes: null, status: 'active' };
    ...
    const logId = randomUUID(); await supabaseRequest('app_data', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ collection: 'auditLogs', id: logId, data: { id: logId, action: 'STUDENT_CHECK_IN', studentId, tabletId: assignedTabletId, timestamp: startedAt } }) });
