...
function normalizePin(value: unknown) { return String(value ?? '').trim().replace(/^PIN[-\s:]*/i, '').trim(); }
...
async function readCollectionServerSide(collection: string): Promise<any[]> {
  ...
  if (!response.ok) {
    ...
    console.error(`Supabase ${collection} read failed:`, response.status, body);
...
async function findStudentById(studentId: string) {
  ...
  return students.find((item) => valueMatches(item?.id ?? item?.studentId ?? item?.studentID, studentId));
}
...
app.post('/api/student/register', async (req, res) => {
  ...
  const assignedTabletId = normalizeTabletId(req.body?.tabletId);
  if (!name || !pin || pin.length < 4 || !standard || !roomNumber || !wingNumber || !assignedTabletId) return res.status(400).json({ error: 'All student registration fields are required.' });
  ...
  try {
    const [students, tablets] = await Promise.all([readCollectionServerSide('students'), readCollectionServerSide('tablets')]);
    if (students.some((item) => containsPin(item, pin))) return res.status(409).json({ error: 'This student PIN is already registered.' });
    if (students.some((item) => valueMatches(item?.assignedTabletId ?? item?.assignedTabletNumber ?? item?.tabletId, assignedTabletId))) return res.status(409).json({ error: 'This tablet is already assigned to a student.' });
    if (tablets.length > 0 && !tablets.some((item) => valueMatches(item?.id ?? item?.tabletId ?? item?.tabletNumber, assignedTabletId))) return res.status(400).json({ error: 'Tablet ID is not registered.' });

    const studentId = randomUUID();
    const student = { id: studentId, name, pinHash: pinHash(pin), standard, coachingType, roomNumber, wingNumber, assignedTabletId, isActive: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    return res.status(201).json({ student: { id: studentId, name, standard, coachingType, roomNumber, wingNumber, tabletId: assignedTabletId } });
  } catch (error) {
    console.error('Student registration failed:', error);
    return res.status(500).json({ error: 'Student registration could not be completed.' });
  }
});
...
app.post('/api/student/activate', async (req, res) => {
  ...
  try {
    ...
    if (!student || student?.isActive === false) return res.status(401).json({ error: 'Tablet ID or PIN is incorrect.' });
    const assignedTabletId = normalizeTabletId(student?.assignedTabletId ?? student?.assignedTabletNumber ?? student?.tabletId ?? '');
    if (!assignedTabletId || assignedTabletId !== tabletId) return res.status(401).json({ error: 'Tablet ID or PIN is incorrect.' });
    ...
    return res.json({ session: { sessionToken, studentId, studentName: session.studentName, tabletId: assignedTabletId, startedAt } });
  } catch (error) {
