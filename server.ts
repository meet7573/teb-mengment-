...
// Lightweight in-memory protection for the student activation endpoint.
// This limits repeated PIN guesses without affecting the existing admin APIs.
...
// Phase 1: student-only tablet activation. It returns only the minimum data
// required by the student UI and never exposes the admin collections.
app.post('/api/student/activate', async (req, res) => {
  ...
  try {
    const [students, tablets, assignments, sessions] = await Promise.all([
      readCollectionServerSide('students'),
      readCollectionServerSide('tablets'),
      readCollectionServerSide('assignments'),
      readCollectionServerSide('studentSessions'),
    ]);

    const tablet = tablets.find((item) =>
      containsKeyValue(item, ['id', 'tabletId', 'tabletid', 'tabletCode', 'tabletName', 'serialNumber', 'serial'], tabletId),
    );
    ...
    const activeForStudent = sessions.some((item) =>
      item?.status === 'active' && valueMatches(item?.studentId, studentId),
    );
    const activeForTablet = sessions.some((item) =>
      item?.status === 'active' && valueMatches(item?.tabletId, tabletKey),
    );
