...
const COLLECTIONS = new Set(['students', 'tablets', 'boxes', 'assignments', 'attendance', 'auditLogs']);
...
function valueMatches(value: unknown, target: string): boolean { return String(value ?? '').trim().toLowerCase() === target.trim().toLowerCase(); }
...
app.post('/api/student/activate', async (req, res) => {
  ...
  try {
    const [students, tablets, assignments, sessions] = await Promise.all([readCollectionServerSide('students'), readCollectionServerSide('tablets'), readCollectionServerSide('assignments'), readCollectionServerSide('studentSessions')]);
    const tablet = tablets.find((item) => containsKeyValue(item, ['id', 'tabletId', 'tabletid', 'tabletCode', 'tabletName', 'serialNumber', 'serial'], tabletId));
    if (!tablet) return res.status(401).json({ error: 'Tablet ID or PIN is incorrect.' });
    const student = students.find((item) => containsKeyValue(item, ['pin', 'pinNo', 'pinno', 'pinNumber', 'studentPin', 'studentpin'], pin));
    if (!student) return res.status(401).json({ error: 'Tablet ID or PIN is incorrect.' });
    const studentId = String(student?.id ?? student?.studentId ?? student?.studentID ?? '');
    const tabletKey = String(tablet?.id ?? tablet?.tabletId ?? tablet?.tabletID ?? tabletId);
    const assigned = assignments.some((assignment) => {
      const studentMatch = studentId && containsKeyValue(assignment, ['studentId', 'studentid', 'studentID'], studentId);
      const tabletMatch = containsKeyValue(assignment, ['tabletId', 'tabletid', 'tabletID'], tabletKey) || containsKeyValue(assignment, ['tabletId', 'tabletid', 'tabletID'], tabletId);
      return Boolean(studentMatch && tabletMatch);
    });
    if (!assigned) return res.status(403).json({ error: 'This student is not assigned to this tablet.' });
    const activeForStudent = sessions.some((item) => item?.status === 'active' && valueMatches(item?.studentId, studentId));
    const activeForTablet = sessions.some((item) => item?.status === 'active' && valueMatches(item?.tabletId, tabletKey));
    ...
    const session = { id: sessionToken, sessionToken, studentId, tabletId: tabletKey, studentName: String(student?.name ?? student?.studentName ?? 'Student'), startedAt, returnedAt: null, durationMinutes: null, status: 'active' };
    ...
    return res.json({ session: { sessionToken, studentName: session.studentName, tabletId: tabletKey, startedAt } });
  } catch (error) {
...
app.post('/api/student/return', async (req, res) => {
  ...
  try {
    ...
    console.error('Student return failed:', error);
    return res.status(500).json({ error: 'Tablet return could not be completed.' });
  }
});
...
async function startServer() {
  if (process.env.NODE_ENV !== 'production') {
    ...
    app.get('/src/assets/images/school_management_logo_1785906402051.jpg', (_req, res) => { res.sendFile(path.join(distPath, 'favicon.svg')); });
