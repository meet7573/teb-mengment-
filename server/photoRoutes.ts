...
function getBucket(kind: unknown): PhotoKind | null {
  return kind === 'student' || kind === 'tablet' ? kind : null;
}
...
export function createPhotoRouter(config: PhotoConfig) {
  ...
  router.post('/upload', async (req, res) => {
    ...
    try {
      const kind = getBucket(req.body?.kind);
      ...
      const path = `${itemId}/${randomUUID()}.${safeExtension(decoded.contentType)}`;
      const response = await storageRequest(supabaseUrl!, supabaseKey!, `object/${BUCKETS[kind]}/${path}`, {
        method: 'POST',
        headers: { 'Content-Type': decoded.contentType, 'x-upsert': 'false' },
        body: decoded.buffer,
      });
      if (!response.ok) {
        const detail = await response.text();
        console.error('Supabase photo upload failed:', response.status, detail);
        return res.status(502).json({ error: 'Photo storage upload failed.' });
      }

      const signed = await storageRequest(supabaseUrl!, supabaseKey!, `object/sign/${BUCKETS[kind]}/${path}`, {
        method: 'POST',
      ...
      return res.status(400).json({ error: error instanceof Error ? error.message : 'Photo upload failed.' });
    }
  });

  router.get('/signed-url', async (req, res) => {
    if (!isAllowedRole(String(req.headers['x-admin-role'] || '')) && !req.query.path) return res.status(403).json({ error: 'Admin access required.' });
    if (!configured()) return res.status(503).json({ error: 'Photo storage is not configured.' });
    try {
      const kind = getBucket(req.query.kind);
      const photoPath = String(req.query.path || '').trim();
      if (!kind || !photoPath || photoPath.includes('..') || photoPath.startsWith('/')) return res.status(400).json({ error: 'Invalid photo path.' });
      const response = await storageRequest(supabaseUrl!, supabaseKey!, `object/sign/${BUCKETS[kind]}/${photoPath}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expiresIn: 3600 }),
      });
      ...
      return res.json({ url: data.signedURL || data.signedUrl });
    } catch (error) {
      console.error('Photo signed URL failed:', error);
      return res.status(500).json({ error: 'Could not create photo URL.' });
    }
  });
  ...
  router.post('/delete', async (req, res) => {
    ...
    try {
      ...
      if (!kind || !photoPath || photoPath.includes('..') || photoPath.startsWith('/')) return res.status(400).json({ error: 'Invalid photo path.' });
      const response = await storageRequest(supabaseUrl!, supabaseKey!, `object/${BUCKETS[kind]}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
