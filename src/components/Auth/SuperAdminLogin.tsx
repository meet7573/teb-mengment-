...
export const SuperAdminLogin: React.FC<Props> = ({ isOpen, onLoginSuccess }) => {
  ...
  const requestOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setOtp('');
      setStep('otp');
      setResendCooldown(Number(data?.resendAfterSeconds || RESEND_COOLDOWN_SECONDS));
      setMessage('A 6-digit OTP has been sent to your authorized email address.');
    } catch (e) {
    ...
    try {
      ...
      const user: AppUser = data.user
        ? {
            ...
            username: data.user.username || SUPER_ADMIN_USERNAME,
            email: data.user.email || SUPER_ADMIN_EMAIL,
            mobileNumber: '',
    ...
    <div className="w-full max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-2xl border border-slate-200 grid lg:grid-cols-[1.05fr_.95fr]">
      ...
      <div className="p-6 sm:p-10 lg:p-12">
        ...
        {step === 'credentials' ? <>
          ...
          <h1 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight text-slate-950">Admin Login</h1>
          <p className="mt-2 text-sm text-slate-500">Enter your authorized user name and email ID to continue.</p>
        </> : <>
          <p className="text-sm font-bold text-indigo-600">VERIFY YOUR EMAIL</p>
          <h1 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight text-slate-950">Enter OTP</h1>
          <p className="mt-2 text-sm text-slate-500">Enter the 6-digit verification code sent to <span className="font-bold text-slate-700">{maskEmail(email)}</span>.</p>
        </>}
        ...
        {step === 'credentials' ? <form onSubmit={requestOtp} className="mt-7 space-y-5">
          ...
          <div className="flex items-center justify-between text-sm">
            <button type="button" onClick={resetToCredentials} disabled={loading} className="inline-flex items-center gap-1.5 font-semibold text-slate-500 hover:text-slate-700 disabled:opacity-50"><ArrowLeft className="w-4 h-4" /> Change details</button>
            <button type="button" onClick={() => requestOtp()} disabled={loading || resendCooldown > 0} className="font-bold text-indigo-600 hover:text-indigo-700 disabled:text-slate-400 disabled:cursor-not-allowed">{resendCooldown > 0 ? `Resend OTP in ${resendCooldown}s` : 'Resend OTP'}</button>
          </div>
