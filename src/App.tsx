...
function MainApp() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => { if ((e.metaKey || e.ctrlKey) && e.key?.toLowerCase() === 'k') { e.preventDefault(); setIsSearchOpen(true); } };
  ...
  return <div className="min-h-screen bg-[var(--bg-color,#F8FAFC)] text-[var(--font-color,#0F172A)] font-sans antialiased selection:bg-indigo-600 selection:text-white flex">
    ...
    <main className={`flex-1 transition-all duration-300 min-w-0 ${sidebarCollapsed ? 'ml-16' : 'ml-16 sm:ml-64'} ${activeTab === 'attendance' ? 'h-screen overflow-hidden p-4 sm:p-6 flex flex-col' : 'p-4 sm:p-6 min-h-screen'}`}>
      {activeTab === 'dashboard' && <DashboardView students={students} tablets={tablets} boxes={boxes} attendanceRecords={attendanceRecords} onNavigate={setActiveTab} />}
      {activeTab === 'attendance' && <DigitalAttendance students={students} attendanceRecords={attendanceRecords} onSaveAttendanceRecords={handleSaveAttendance} activeRole={activeRole} onNavigate={setActiveTab} />}
      {activeTab === 'students' && <StudentManagement students={students} onSaveStudents={handleSaveStudents} activeRole={activeRole} onNavigate={setActiveTab} onQuickAssignTablet={handleQuickAssignFromStudent} />}
      {activeTab === 'boxes' && <TabletBoxManagement boxes={boxes} tablets={tablets} students={students} onSaveBoxes={handleSaveBoxes} onSaveTablets={handleSaveTablets} onSaveStudents={handleSaveStudents} activeRole={activeRole} onNavigate={setActiveTab} />}
      {activeTab === 'tablets' && <TabletManagement tablets={tablets} students={students} boxes={boxes} onSaveTablets={handleSaveTablets} onSaveBoxes={handleSaveBoxes} activeRole={activeRole} onNavigate={setActiveTab} onQuickAssign={handleQuickAssignFromTablet} />}
      {activeTab === 'assignments' && <TabletAssignmentView assignments={assignments} students={students} tablets={tablets} onSaveAssignments={handleSaveAssignments} onSaveStudents={handleSaveStudents} onSaveTablets={handleSaveTablets} activeRole={activeRole} onNavigate={setActiveTab} preselectedStudentForAssign={preselectedStudent} preselectedTabletForAssign={preselectedTablet} onClearPreselections={() => { setPreselectedStudent(null); setPreselectedTablet(null); }} />}
      {activeTab === 'reports' && <ReportsView students={students} tablets={tablets} boxes={boxes} attendanceRecords={attendanceRecords} activeRole={activeRole} onNavigate={setActiveTab} />}
      ...
      {activeTab === 'settings' && <SettingsView activeRole={activeRole} onNavigate={setActiveTab} onRoleChange={handleRoleChange} onResetData={handleResetData} />}
    </main>
    <GlobalSearchModal students={students} tablets={tablets} boxes={boxes} assignments={assignments} isOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} onNavigate={setActiveTab} />
...
export default function App() {
  if (window.location.pathname === '/student' || window.location.pathname.startsWith('/student/')) return <StudentTabletApp />;
