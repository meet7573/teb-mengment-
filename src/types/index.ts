...
export interface Student { id: string; name: string; email?: string; emailApproved?: boolean; pinNumber: string; studentPin?: string; standard: StandardGrade; isCoachingStudent: boolean; status: StudentStatus; assignedTabletId?: string; assignedTabletNumber?: string; photoPath?: string; roomNumber?: string; wingNumber?: string; createdAt: string; }
...
export type TabletMovementStatus = 'PENDING' | 'APPROVED' | 'REJECTED' | 'OUTSIDE' | 'RETURNED' | 'OVERDUE';
export interface TabletMovement { id: string; assignmentId: string; studentId: string; studentName: string; pinNumber: string; tabletId: string; tabletName: string; tabletNumber: string; movementType: string; requestDate: string; startDate: string; expectedReturnDate: string; destination: string; reason: string; notes?: string; status: TabletMovementStatus; createdAt: string; updatedAt: string; eventId?: string; eventName?: string; }
export interface TabletMovementEvent { id: string; eventName: string; startDate: string; endDate: string; notes?: string; studentIds: string[]; createdAt: string; updatedAt: string; }
...
export interface AuditLog { id: string; timestamp: string; userName: string; userRole: UserRole; action: string; module: 'Students' | 'Tablets' | 'Tablet Boxes' | 'Assignments' | 'Attendance' | 'Tablet Movement' | 'System'; details: string; }
