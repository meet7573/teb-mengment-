...
export function StudentTabletApp() {
  ...
  async function login() {
    ...
    try {
      await activate();
      setMessage('Student signed in successfully.');
    } catch (e) {
  ...
  async function register() {
    ...
    try {
      ...
      setMode('login');
      setMessage('Student registered successfully. Admin must assign a Tablet ID before you can start.');
    } catch (e) {
        ...
        {!session ? (
          <>
            <div className="mb-5 grid grid-cols-2 rounded-xl bg-slate-100 p-1">
              <button type="button" onClick={() => { setMode('login'); setError(''); }} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === 'login' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>Existing Student</button>
              <button type="button" onClick={() => { setMode('register'); setError(''); }} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === 'register' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>First Time</button>
            </div>
          ...
          <div className="space-y-5">
            ...
            <button type="button" disabled={loading} onClick={returnTablet} className="w-full rounded-xl bg-slate-900 px-4 py-3.5 font-semibold text-white disabled:opacity-60">{loading ? 'Checking out...' : 'CHECKOUT / RETURN TABLET'}</button>
          </div>
        )}
