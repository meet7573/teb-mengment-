...
async function fetchCollection(collectionName: string): Promise<any[]> { const response = await fetch(`/api/db/${collectionName}`); if (!response.ok) throw new Error(`Database read failed: ${response.status}`); return response.json(); }
...
export function subscribeToCollection<T>(collectionName: string, callback: (data: T[]) => void) {
  ...
  fetchCollection(collectionName).then((data) => setLocalData(collectionName, data)).catch((error) => { if (import.meta.env.PROD) { console.error(`Failed to load ${collectionName} from the database`, error); callback([]); } else callback(getLocalData(collectionName)); });
...
export async function syncCollection<T extends { id: string }>(collectionName: string, _current: T[], updated: T[]) {
  ...
  if (!response.ok) { if (!import.meta.env.PROD) { setLocalData(collectionName, updated); return; } throw new Error(`Database save failed: ${response.status}`); }
  setLocalData(collectionName, updated);
}

export async function resetPersistentDatabase() {
  const response = await fetch('/api/db', { method: 'DELETE' }); if (!response.ok) throw new Error(`Database reset failed: ${response.status}`);
